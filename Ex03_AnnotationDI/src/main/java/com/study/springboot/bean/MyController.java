...
public class MyController
{
	...
	// 빈이 생성될때 member1변수가 참조할 객체를 자동으로 찾아온다.
	@Autowired
	Member member1;
	...
	@Autowired
	// 유사한 객체가 printerA, printerB등 여러개일때 빈의 이름으로 정확하게 지정한다.
	@Qualifier("printerB")
	Printer printer;
	@Autowired
	Member member2;
	...
	@GetMapping("/")
	// @ResponseBody 어노테이션은 json이나 Xml등 rest api 형태의 응답을 할 경우
	...
	public @ResponseBody String root()
	{
		...
		// 3.싱글톤인지 확인
		if (member1 == member2) { // true
			System.out.println("동일한 객체입니다.");
		} else {
			System.out.println("서로 다른 객체입니다.");
		}
		
		return "Annotaion 사용하기";
	}
}
